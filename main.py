...
async def handle_all_callbacks(event: MessageCallback):
    ...
    elif payload == "program_show":
        await handle_program_show(event)
...
async def handle_program_show(event: MessageCallback):
    """Показ подробной программы форума блоками"""
    print(f"[DEBUG] handle_program_show: отправка программы форума")
    # Удаляем сообщение с кнопкой
    message_id = get_message_id_from_event(event)
    if message_id:
        await delete_message(message_id)
    chat_id = get_chat_id_from_event(event)
    back_buttons = [[{"type": "callback", "text": "⬅️ Назад в меню", "payload": "show_menu"}]]
    block1 = (
    ...
    for part in (block1, block2, block3, block4):
        await send_message_with_buttons(chat_id, part, back_buttons)
